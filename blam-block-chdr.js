const blam_content_type = Object.freeze({ // int8_t
   none:        -1,
   dlc:          0,
   save:         1,
   screenshot:   2,
...
const blam_activity = Object.freeze({ // int8_t
   none:        -1,
   activities:   0,
   campaign:     1,
   survival:     2,
...
const blam_game_mode = Object.freeze({ // uint8_t
   none:        0,
   campaign:    1,
...
const blam_game_engine = Object.freeze({ // uint8_t
   none:      0,
   forge:     1,
   megalo:    2,
   campaign:  3,
...
class BlamBlockContentHeader {
   constructor(stream) {
      ...
      this.contentType = stream.readSInt8(); // blam_content_type
      stream.skipBytes(3);
      ...
      this.activity = stream.readSInt8(); // blam_activity
      this.gameMode = stream.readUInt8(); // blam_game_mode
      this.engine   = stream.readUInt8(); // blam_game_engine
      stream.skipBytes(1);
